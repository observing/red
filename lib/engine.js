...
function Engine (options, encode, decode) {
  this.namespace = 'RED';
...
Engine.prototype.connect = function connect (done) {
  var self = this
    , finish = this.async(3, function finish (err) {
        if (err) {
...
Engine.prototype.encode = function encode (data) {
...
Engine.prototype.decode = function decode (data) {
...
Engine.prototype.publish = function publish (channel, message) {
  ...
  this.pub.publish(pattern, this.encode(message));
};
...
Engine.prototype.listen = function listen () {
  ...
  this.sub.on('pmessage', function pmessage (pattern, channel, message) {
    ...
    self.emit(channel, self.decode(message), pattern);
  });
...
Engine.prototype.close = function close () {
  ...
  this._events.error = errorListeners;
