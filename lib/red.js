...
RED.prototype.request = function request (req, res, head) {
  ...
  if (data.session) {
    if (!util.id.test(this.secret, data.session, data)) {
      return this.invalid(res, 'invalid session');
    }

    this.authorized(data.session, function authorized (err, valid) {
      if (err) return self.invalid(res, 'authorization error');
      if (!valid) return self.invalid(res)
    })
  }
};
