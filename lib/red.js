...
RED.prototype.listen = function listen (http, options, fn) {
  ...
  this.requestListeners = this.server.listeners('request').splice(0);
  ...
  // initialize the procol parsers
  Object.keys(Protocol).forEach(function initialize (version) {
    // don't process the stream interfaces or other interfaces that are exposed
    // throught the Protocol object.
    if (!+version) return;

    var protocol = self.protocol[version] = new Protocol[version];

    // dispatch message
    protocol.on('message', self.message.bind(self, protocol));
  });
...
/**
 * @param {Protocol} protocol reference to the used protocol parser
...
RED.prototype.message = function message (protocol, type, data, id, raw, sessionid) {
  ...
  switch (type) {
    case 'event':
      if (this.blacklisted(data.name)) return false;

      data.args.unshift(data.name);
      this.emit.apply(socket, data.args);
      break;

    case 'message':
    case 'codec':
      this.emit.call(socket, 'message', data);
      break;

    case 'disconnect':
      this.engine.expire('connection id', 0);
      // @TODO find the transport here, and disconnect it
      break;
  }
...
/**
 * Checks if a given event is blacklisted or not.
 *
 * @param {String} name
 * @returns {Boolean}
 * @api public
...
RED.prototype.blacklisted = function blacklisted (name) {
  if (~this.blacklist.indexOf(name)) return true;
  if (name.indexOf(this.namespace + ':') === 0) return true;
  if (name.indexOf('RED:') === 0) return true;
...
RED.prototype.generate = function generate (request) {
  var uri = url.parse(request.url, true)
    , paths = uri.pathname.replace(this.endpoint, '').match(RED.parsePath);
...
RED.prototype.request = function incomingRequest (request, response, head) {
  ...
  if (!this.protocol[data.protocol]) {
...
RED.prototype.handshake = function handshake (request, response, data) {
  ...
  // answer the thingy
  this.JSONP(data, response, this.protocol[data.protocol].encode('handshake',data));
  return this;
};
...
RED.prototype.transport = function transport (data, request, response, head) {
  if (this.transports[data.id]) {
    ...
    if (request.method !== 'POST') {
      ...
      this.transports[data.id].initialize(request, response, head);
      return this;
    }
  }
  ...
  this.transports[data.id] = new Transports[data.transport](this.engine, response, {
    ...
    , protocol: this.protocol[data.protocol]
    , logger: this.logger
  });
...
RED.HTTP = function http(options) {
  ...
  server.on('request', function req (request, response) {
    // build up the response
    response.setHeader('X-Powered-By', 'RED v' + exports.version);
    ...
    try {
      ...
      response.end('RED> 404. That\'s an error');
    } catch (e) {}
  });
